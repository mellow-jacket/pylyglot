...
from ..autobox import md_image_cleaner
...
def scrape(self):
    ...
    def test_conditions(url):
        ...
        out = True
        if not any([ x in url for x in ['.jpeg','.jpg','.webp']]):
            out = False
        if not url.startswith(('http:', 'https:')):
            out = False  
        if 'Skeleton%20Soldier' not in url:
            out = False
        return out
    ...
    cleaner = md_image_cleaner(in_directory=self.path['scraped_image'], out_directory=['raw_image'])
    cleaner.split_raws()
